...
export const AgentePage: React.FC = () => {
  ...
  return (
    <div className="w-full h-[100dvh] bg-slate-50 flex flex-col font-sans text-slate-900 overflow-hidden">
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-50 flex flex-col gap-2 w-full max-w-sm px-4 pointer-events-none">
        {current.error && (
          <div className="pointer-events-auto p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl shadow-xl flex justify-between animate-in slide-in-from-top-2">
            ...
            <button onClick={() => setAgentState(activeTab, { error: null })}>✕</button>
          </div>
        )}
        {current.success && (
      ...
      {/* Header fijo */}
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 md:px-6 py-4 flex-none z-20">
        <div className="max-w-5xl mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center shadow-slate-200 shadow-md text-white">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {current.dirty && (
                    <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-amber-50 text-amber-700 border border-amber-200">
                ...
                <p className="text-xs text-slate-500 font-medium truncate">{tabMeta.subtitle}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="group flex items-center gap-2 px-4 md:px-5 py-2.5 bg-slate-900 hover:bg-blue-600 text-white font-medium rounded-xl shadow-lg shadow-slate-200 hover:shadow-blue-200 transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100"
                ...
                {TABS.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "relative flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold transition-all",
                        isActive
                    ...
                    >
                      <span
                        className={[
                          ...
                          isActive ? "bg-blue-50 text-blue-700" : "bg-white/60 text-slate-600",
                        ].join(" ")}
                      >
                        {t.icon}
                      </span>
                      <span>{t.label}</span>
                      {st.dirty && <span className="ml-1 w-2 h-2 rounded-full bg-amber-500" />}
                    </button>
                  );
                })}
              </div>
